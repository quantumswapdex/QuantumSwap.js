 ...
 * 7) Quote (getAmountsOut / getAmountsIn)
 * 8) Add liquidity (1000 BigCat, 1000 SmallDog)
...
// Use parent's quantumcoin so Initialize() and quantumswap use the same instance (avoids "SDK not initialized" when examples/node_modules has its own quantumcoin)
...
async function main() {
  ...
  // Require quantumswap from repo root so it uses parent's quantumcoin (same instance we initialized)
  ...
  const {
    ...
    QuantumSwapV2Pair,
  } = require(parentQswapPath);
  const provider = new JsonRpcProvider(RPC_URL, CHAIN_ID);
  // Use parent's Wallet with test credentials (do not use _test-wallet so we don't load examples' quantumcoin)
  ...
  */
  ...
  if (Array.isArray(amountsOut) && amountsOut.length >= 2) {
    console.log("7) Quote getAmountsOut(1 BigCat):", amountsOut[1].toString(), "SmallDog");
  }
  let amountsIn = [];
  try {
  ...
  // --- 8) Add liquidity (1000 BigCat, 1000 SmallDog) ---
  if (pairCreated) {
    console.log("8) Adding liquidity (1000 BigCat, 1000 SmallDog)...");
