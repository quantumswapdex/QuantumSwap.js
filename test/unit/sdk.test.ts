 ...
 * @description TypeScript mirror of sdk.test.js — platform-independent, no RPC.
 *
 * Runs under `node --require ts-node/register --test` (transpile-only). Verifies
 * the SDK loads, exposes expected exports, carries well-formed ABIs/bytecode,
 * constructs wrappers, and encodes calldata via quantumcoin's Interface — from
 * TypeScript, confirming the published types/entry resolve for TS consumers.
 */
...
describe("QuantumSwap SDK (non-transactional, TS)", () => {
  ...
  it("populateTransaction encodes ERC20 calldata", async (t) => {
    if (!initialized) {
